...
public partial class Control_Add_Location : UserControl
{
    ...
    private async void Control_Add_Location_Button_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Control_Add_Location_ComboBox_Building.SelectedIndex <= 0 || Control_Add_Location_ComboBox_Building.SelectedItem?.ToString() == "Select Building")
            {
            ...
            string building = Control_Add_Location_ComboBox_Building.SelectedItem?.ToString() ?? string.Empty;

            if (await Dao_Location.LocationExists(location))
            {
                MessageBox.Show($"Location '{location}' already exists.", "Duplicate Location", MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                Control_Add_Location_TextBox_Location.Focus();
                return;
            }
