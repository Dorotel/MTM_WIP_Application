...
namespace MTM_Inventory_Application.Controls.SettingsForm
{
    public partial class RemoveOperationControl : UserControl
    {
        ...
        private async void RemoveButton_Click(object sender, EventArgs e)
        {
            if (_currentOperation == null)
            {
                MessageBox.Show("Please select an operation to remove.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            var operationNumber = _currentOperation["Operation"]?.ToString() ?? string.Empty;
            var result = MessageBox.Show($"Are you sure you want to remove the operation number '{operationNumber}'?{Environment.NewLine}{Environment.NewLine}This action cannot be undone.", "Confirm Removal", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            ...
            try
            {
                ...
                MessageBox.Show("Operation removed successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error removing operation: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
