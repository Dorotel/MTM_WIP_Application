    ...
    private async void RemoveButton_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var result = MessageBox.Show(
                $@"Are you sure you want to remove the item type '{itemType}'?{Environment.NewLine}{Environment.NewLine}This action cannot be undone.",
                @"Confirm Removal",
            ...
            if (result == DialogResult.Yes)
            {
                ...
                // Notify that item type was removed
                ItemTypeRemoved?.Invoke(this, EventArgs.Empty);

                MessageBox.Show(@"Item type removed successfully.", @"Success",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        catch (Exception ex)
        {
            MessageBox.Show($@"Error removing item type: {ex.Message}", @"Error",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
