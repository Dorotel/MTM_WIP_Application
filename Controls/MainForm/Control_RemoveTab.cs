...
Are you sure?",
    ...
    private async void Control_RemoveTab_Button_Reset_Click()
    {
        ...
        try
        {
            ...
            // 2) Unbind DataSource before DataTable reset & Update status strip to show reset is in progress
        ...
        catch (Exception ex)
        {
            LoggingUtility.LogApplicationError(ex);
            _ = Dao_ErrorLog.HandleException_GeneralError_CloseApp(ex, false,
                new StringBuilder().Append("MainForm_Remove_Button_Reset").ToString());
        }
        finally
        {
            if (MainFormInstance != null)
