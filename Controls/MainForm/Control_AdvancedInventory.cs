    ...
    public partial class Control_AdvancedInventory : UserControl
    {
        ...
        public void SetProgressControls(ToolStripProgressBar progressBar, ToolStripStatusLabel statusLabel)
        {
            _progressHelper = Helper_StoredProcedureProgress.Create(progressBar, statusLabel, 
                this.FindForm() ?? throw new InvalidOperationException(
                    $"Control '{this.Name ?? this.GetType().Name}' must be added to a Form before calling SetProgressControls. " +
                    "Ensure that the control is added to a Form and visible in the control hierarchy.");
        }
        ...
        private async Task AdvancedInventory_Single_HardResetAsync()
        {
            ...
            try
            {
                ...
                MainFormInstance?.TabLoadingControlProgress?.UpdateProgress(10,
                    "Resetting Advanced Inventory (Single)...");
        ...
        private async Task AdvancedInventory_MultiLoc_HardResetAsync()
        {
            ...
            try
            {
                MainFormInstance?.TabLoadingControlProgress?.ShowProgress();
                MainFormInstance?.TabLoadingControlProgress?.UpdateProgress(10,
                    "Resetting Advanced Inventory (MultiLoc)...");
