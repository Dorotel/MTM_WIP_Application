...
public partial class Control_AdvancedRemove : UserControl
{
    ...
    private async void Control_AdvancedRemove_Button_Search_Click(object? sender, EventArgs? e)
    {
        try
        {
            ...
            var part = Control_AdvancedRemove_ComboBox_Part.Text.Trim();
            var op = Control_AdvancedRemove_ComboBox_Op.Text.Trim();
            var loc = Control_AdvancedRemove_ComboBox_Loc.Text.Trim();
            var qtyMinText = Control_AdvancedRemove_TextBox_QtyMin.Text.Trim();
            var qtyMaxText = Control_AdvancedRemove_TextBox_QtyMax.Text.Trim();
            var notes = Control_AdvancedRemove_TextBox_Notes.Text.Trim();
            var user = Control_AdvancedRemove_ComboBox_User.Text.Trim();
            ...
            var locSelected = Control_AdvancedRemove_ComboBox_Loc.SelectedIndex > 0 && !string.IsNullOrWhiteSpace(loc);
...
{summary}Are you sure?",
    ...
    private async void Control_AdvancedRemove_Button_Reset_Click(object? sender, EventArgs? e)
    {
        ...
        try
        {
            Debug.WriteLine("[DEBUG] AdvancedRemove Reset button clicked - start");
            ...
            Debug.WriteLine("[DEBUG] AdvancedRemove Reset button clicked - end");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[ERROR] Exception in AdvancedRemove Reset: {ex}");
            LoggingUtility.LogApplicationError(ex);
            _ = Dao_ErrorLog.HandleException_GeneralError_CloseApp(ex, false,
        ...
        {
            Debug.WriteLine("[DEBUG] AdvancedRemove Reset button re-enabled");
