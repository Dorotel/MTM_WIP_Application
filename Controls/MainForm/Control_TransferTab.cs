...
public partial class ControlTransferTab : UserControl
{
    ...
    private async void Control_TransferTab_Button_Reset_Click()
    {
        ...
        try
        {
            Debug.WriteLine("[DEBUG] TransferTab Reset button clicked - start");
            ...
            Debug.WriteLine("[DEBUG] TransferTab Reset button clicked - end");
        }
        catch (Exception ex)
        {
            ...
            LoggingUtility.LogApplicationError(ex);
            _ = Dao_ErrorLog.HandleException_GeneralError_CloseApp(ex, false,
                new StringBuilder().Append("MainForm_Transfer_Button_Reset").ToString());
        }
        finally
        {
            Debug.WriteLine("[DEBUG] TransferTab Reset button re-enabled");
            Control_TransferTab_Button_Reset.Enabled = true;
            // 8) Set Search and Transfer buttons to their default state
            if (MainFormInstance != null)
            {
