...
# Script to apply renaming transformations
...
for file in "Forms/Transactions/Transactions.cs" "Forms/Transactions/Transactions.Designer.cs"; do
    if [[ -f "$file" ]]; then
        ...
        for old_name in "${!MAPPINGS[@]}"; do
            ...
            if [[ $count -gt 0 ]]; then
                echo "  Renaming $count occurrences of '$old_name' -> '$new_name'"
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Renamed '$old_name' to '$new_name' in $file ($count occurrences)" >> "$LOG_FILE"
        ...
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Successfully updated file: $file" >> "$LOG_FILE"
    else
...
echo "Mode: EXECUTE"
echo "Renaming complete! Check log file: $LOG_FILE"
echo "============================"