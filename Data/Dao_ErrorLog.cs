...
internal static class Dao_ErrorLog
{
        ...
        bool useAsync = false)
    {
        ...
        try
        {
            using var reader = useAsync
                ? await HelperMySql.ExecuteReader("SELECT DISTINCT `MethodName`, `ErrorMessage` FROM `log_error`",
                    useAsync: true)
                : HelperMySql.ExecuteReader("SELECT DISTINCT `MethodName`, `ErrorMessage` FROM `log_error`").Result;
    ...
    private static async Task ExecuteNonQueryAsync(string sql, Dictionary<string, object>? parameters, bool useAsync)
    {
        ...
        {
            if (parameters == null)
                await HelperMySql.ExecuteNonQuery(sql, useAsync: useAsync);
            else
        ...
        bool useAsync)
    {
        ...
        {
            ["@User"] = Core_WipAppVariables.User,
            ["@Severity"] = severity,
            ["@ErrorType"] = errorType,
