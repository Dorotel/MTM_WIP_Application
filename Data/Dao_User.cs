    ...
    internal static class Dao_User
    {
        ...
        public static async Task<string> GetSettingsJsonAsync(string field, string user, bool useAsync)
        {
            ...
            try
            {
                ...
                using MySqlCommand cmd = new("usr_ui_settings_GetSettingsJson_ByUserId", conn)
                {
                ...
                using MySqlCommand legacyCmd = new("usr_users_GetUserSetting_ByUserAndField", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };

                legacyCmd.Parameters.AddWithValue("p_User", user);
                legacyCmd.Parameters.AddWithValue("p_Field", field);
        ...
        internal static async Task<int> GetUserRoleIdAsync(int userId, bool useAsync = false)
        {
            ...
            try
            {
                Dictionary<string, object> parameters = new() { ["p_UserID"] = userId };
                DataTable result = await HelperDatabaseCore.ExecuteDataTable(
