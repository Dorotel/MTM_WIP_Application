...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `inv_inventory_Remove_Item`(IN `p_PartID` VARCHAR(300), IN `p_Location` VARCHAR(100), IN `p_Operation` VARCHAR(100), IN `p_Quantity` INT, IN `p_ItemType` VARCHAR(100), IN `p_User` VARCHAR(100), IN `p_BatchNumber` VARCHAR(100), IN `p_Notes` VARCHAR(1000))
...
WHERE PartID = p_PartID
  ...
  AND (Notes = p_Notes OR (Notes IS NULL AND p_Notes IS NULL))
LIMIT 1$$
DELIMITER ;
...
BEGIN
    ...
    WHERE PartID = p_PartID
      ...
      AND BatchNumber = p_BatchNumber
      AND (
      (Notes IS NULL AND p_Notes IS NULL) OR
      (Notes = p_Notes)
    )
    LIMIT 1;
    ...
    ELSE
        SET p_Status = 0;
        -- You can add more logic here to check for specific reasons
        IF NOT EXISTS (
        ...
        ELSE
            SET p_ErrorMsg = 'Delete failed due to unknown reason or constraint.';
        END IF;
    END IF;
END$$
DELIMITER ;
