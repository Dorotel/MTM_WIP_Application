...
BEGIN
    ...
    ELSE
        SET @sql = CONCAT('SELECT `', p_Field, '` as FieldValue FROM usr_users WHERE User = ? LIMIT 1');
...
BEGIN
    ...
    SET p_Status = 0;
    SET p_ErrorMsg = CONCAT('Setting ', p_Field, ' updated successfully for user: ', p_User);
