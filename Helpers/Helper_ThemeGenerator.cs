...
/// </summary>
public class Helper_ThemeGenerator
{
    private readonly List<ThemeDefinition> _themes;
    private readonly string _outputDirectory;
    private const string HighContrastForeColor = "#1A1A1A";

    public Helper_ThemeGenerator(string outputDirectory = "Generated_Themes")
    {
    ...
    public void GenerateAllThemes()
    {
        ...
        foreach (var theme in _themes) GenerateThemeSQL(theme);
    ...
    /// </summary>
    private async void GenerateThemeSQL(ThemeDefinition theme)
    {
        var colors = CreateThemeColors(theme);
        var json = SerializeToJson(colors);

        // Insert theme into SQL server (app_themes table)
        var parameters = new Dictionary<string, object>
        {
            ["@ThemeName"] = theme.Name,
            ["@SettingsJson"] = json
        };
        await Data.Dao_User.HelperDatabaseCore.ExecuteNonQuery(
            "INSERT INTO app_themes (ThemeName, SettingsJson) VALUES (@ThemeName, @SettingsJson)",
            parameters,
            true,
    ...
    private string CreateSQLInsert(string themeName, string json)
    {
        ...
        sql.AppendLine("-- Generated automatically by Helper_ThemeGenerator");
    ...
    /// <summary>
    /// Create a ThemeDefinition from a base color and theme name (for custom themes)
    /// </summary>
    public static ThemeDefinition CreateThemeFromBaseColor(Color baseColor, string themeName)
    {
        // Simple algorithm: use base color and adjust brightness for other theme colors
        ...
        {
            Name = string.IsNullOrWhiteSpace(themeName) ? "CustomTheme" : themeName,
            FormBackColor = Lighten(baseColor, 0.8f),
            InputBackColor = Lighten(baseColor, 1.0f),
            ButtonBackColor = Lighten(baseColor, 0.6f),
            HeaderBackColor = Lighten(baseColor, 0.4f),
            AlternateRowColor = Lighten(baseColor, 0.9f),
            BorderColor = Darken(baseColor, 0.2f),
            AccentColor = baseColor,
            AccentColorLight = Lighten(baseColor, 0.3f),
            AccentColorDark = Darken(baseColor, 0.3f),
            ErrorColor = Color.FromArgb(229, 115, 115),
            WarningColor = Color.FromArgb(255, 167, 38),
            SuccessColor = Color.FromArgb(102, 187, 106),
            InfoColor = Color.FromArgb(51, 153, 255)
        };
    }
