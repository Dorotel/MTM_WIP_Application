...
public static class Helper_SqlVariables
{
    /// <summary>
    ...
    /// <param name="uid">The user ID. Defaults to Core_WipAppVariables.User if null or empty.</param>
    ...
    public static string GetConnectionString(string? server, string? database, string? uid, string? password)
    {
        try
        {
            ...
            uid ??= Core_WipAppVariables.User != null ? Core_WipAppVariables.User.ToUpper() : "";


            return
                $"SERVER={server};DATABASE={database};UID={uid};Allow User Variables=True ;";
        }
        catch (Exception ex)
        {
    ...
    public static string GetLogFilePath(string server, string userName)
    {
        var logDirectory = server switch
        {
            ...
            "localhost" => @"C:\Model_Users\johnk\OneDrive\Documents\Work Folder\WIP App Logs",
