...
#region Core_DgvDesigner

/// <summary>
/// 
/// Testing Passed: 05/31/2025
/// 
/// Core_DgvDesigner provides utilities for applying visual themes to Windows Forms and DataGridView controls.
/// 
/// Features:
...
/// - Integrates with Core_WipAppVariables for theme selection and AppLogger for logging actions and errors.
/// 
/// Usage:
...
/// </summary>
public class Core_DgvDesigner
{
    #region Theme Application

    public static void ApplyTheme(Form form)
    {
        var requestedThemeName = Core_WipAppVariables.WipDataGridTheme ?? "Default (Black and White)";
        var theme = Core_AppThemes.GetTheme(requestedThemeName);
        var themeName = Core_AppThemes.ThemeExists(requestedThemeName)
            ? requestedThemeName
        ...
        form.Font = theme.FormFont ?? new Font(form.Font.Name, Core_WipAppVariables.ThemeFontSize, form.Font.Style);
    ...
    private static void ApplyThemeToControls(Control.ControlCollection controls, Core_AppThemes.AppTheme theme)
    {
        foreach (Control ctrl in controls)
        {
            ...
            else
            {
                ...
                ctrl.Font = theme.FormFont ??
                            new Font(ctrl.Font.Name, Core_WipAppVariables.ThemeFontSize, ctrl.Font.Style);
    ...
    private static void ApplyControlSpecificTheme(Control control, Core_AppThemes.AppTheme theme)
    {
        switch (control)
        {
            case Button btn:
    ...
    public static void ApplyThemeToDataGridView(DataGridView dataGridView, Core_AppThemes.AppTheme theme)
    {
    ...
    {
        return Core_AppThemes.GetThemeNames();
    }

    public static string GetEffectiveThemeName()
    {
        return Core_AppThemes.GetEffectiveThemeName();
    }

    public static bool ThemeExists(string themeName)
    {
        return Core_AppThemes.ThemeExists(themeName);
    }

    public static Color GetThemeColor(string themeName, string propertyName)
    {
        return Core_AppThemes.GetThemeColor(themeName, propertyName);
    }

    public static float GetDefaultFontSize()
    {
        return Core_WipAppVariables.ThemeFontSize;
    }
    ...
    {
        if (!Core_AppThemes.ThemeExists(themeName))
            throw new InvalidOperationException($"Theme '{themeName}' not found.");

        return Core_AppThemes.GetTheme(themeName);
    }
    ...
    public static void ApplyThemeToControlForTesting(Control control, string themeName)
    {
        ...
        control.Font = theme.FormFont ??
                       new Font(control.Font.Name, Core_WipAppVariables.ThemeFontSize, control.Font.Style);
        ...
        params string[] propertyNames)
    {
        ...
        {
            var themeProperty = typeof(Core_AppThemes.AppTheme).GetProperty(propName)
