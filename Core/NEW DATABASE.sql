...
--
-- Host: localhost:3306
-- Generation Time: Jun 02, 2025 at 01:48 PM
...
--
-- Triggers `inv_transaction`
--
DELIMITER $$
CREATE TRIGGER `trg_manage_last_10_transactions` AFTER INSERT ON `inv_transaction` FOR EACH ROW BEGIN
    -- Insert the new transaction into last 10
    INSERT INTO sys_last_10_transactions (`User`, `PartID`, `Operation`, `Quantity`)
    VALUES (NEW.`User`, NEW.`PartID`, NEW.`Operation`, NEW.`Quantity`);
    
    -- Delete older transactions beyond 10 per user
    DELETE FROM sys_last_10_transactions
    WHERE `User` = NEW.`User` AND ID NOT IN (
        SELECT ID FROM (
            SELECT ID
...
CREATE TABLE `log_error` (
  ...
  `Severity` enum('Information','Warning','Error','Critical') NOT NULL DEFAULT 'Error',
  `ErrorType` varchar(100) DEFAULT NULL COMMENT 'e.g., DatabaseError, ApplicationError',
  `ErrorMessage` text NOT NULL,
  `StackTrace` text,
  `ModuleName` varchar(200) DEFAULT NULL COMMENT 'Application component/form/module',
  `MethodName` varchar(200) DEFAULT NULL COMMENT 'Method where error occurred',
  `AdditionalInfo` text COMMENT 'Any JSON or other contextual data',
...
CREATE TABLE `md_item_types` (
  ...
  `Issued By` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `sys_roles` (
  ...
  `Permissions` varchar(1000) DEFAULT NULL COMMENT 'JSON or comma-separated list of permissions',
...
ALTER TABLE `md_part_ids`
  ADD PRIMARY KEY (`ID`),
  ADD UNIQUE KEY `uq_item_number` (`Item Number`);
...
ALTER TABLE `sys_last_10_transactions`
  ADD PRIMARY KEY (`ID`),
  ADD KEY `idx_user_datetime` (`User`,`DateTime`);
